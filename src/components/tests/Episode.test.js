...
test("renders the summury test passed as prop", ()=>{
    render(<Episode episode={testEpisode}/>);
    const renderSummary = screen.queryByText(/test expect/i);
    expect(renderSummary).toBeInTheDocument();
    expect(renderSummary).not.toBeNull();
    expect(renderSummary).not.toBeFalsy();
    expect(renderSummary).toBeTruthy();
    expect(renderSummary).toHaveTextContent("test expect");

});

test("renders default image when image is not defined", ()=>{
    render(<Episode episode={testEpisodeWithoutImage}/>);
    const emptyImage = screen.queryByAltText('./stranger_things.png');
